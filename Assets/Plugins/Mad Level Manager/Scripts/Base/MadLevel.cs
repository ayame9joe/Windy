...
public class MadLevel  {
    ...
    public static string currentLevelName {
        get {
            if (!Application.isPlaying || _currentLevelName == null) { // always find for editor
                FindCurrentSceneLevel();
            }
            ...
            if (!extensionDefined) {
                ...
                if (level != null) {
                    ...
                    if (currentExtension != null) {
                        // if has the extension, the progress should be set on the given level
    ...
    static void FindCurrentSceneLevel() {
        ...
        if (Application.isPlaying) {
            ...
            if (!string.IsNullOrEmpty(EditorApplication.currentScene)) {
                levelName = Path.GetFileNameWithoutExtension(EditorApplication.currentScene);
            }
#endif
        }

        var level = activeConfiguration.FindFirstForScene(levelName, out hasMany);
        ...
        if (level != null) {
            ...
            if (Application.isPlaying) {
                Debug.LogError("Mad Level Manager: Cannot find scene " + levelName
                               +
    ...
    static bool CheckLevelLoading() {
        if (Application.isLoadingLevel) {
            Debug.LogWarning("You're already loading a level. " +
                             "Please make sure that your loading method is not executed " +
                             "more than once (e.g. in Update() function).");
            return false;
        }
