...
public class MadLevelConfiguration : ScriptableObject {
    ...
    public class Level : MadLevelScene {
        ...
        /// <summary>
        /// Gets the level scene and extensions after scene in order. Useful for building a game with only
        /// one level for testing purposes.
        /// </summary>
        public List<MadLevelScene> StandaloneScenes() {
            var scenes = new List<MadLevelScene>();

            scenes.Add(this);

            if (extension != null) {
                scenes.AddRange(extension.scenesAfter);
            }
